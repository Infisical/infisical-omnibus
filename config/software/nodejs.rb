...
build do
  ...
  copy "bin/node", "#{install_dir}/bin/"
  copy "bin/npm", "#{install_dir}/bin/"
  copy "bin/npx", "#{install_dir}/bin/"
  command "cp -r lib/node_modules #{install_dir}/lib/"
  
  # Set up npm and npx wrappers
  block do
    File.open("#{install_dir}/bin/npm", 'w') do |file|
      file.print <<-EOH
#!/bin/sh
export NODE_PATH="#{install_dir}/lib/node_modules"
exec "#{install_dir}/bin/node" "#{install_dir}/lib/node_modules/npm/bin/npm-cli.js" "$@"
      EOH
    end
    
    File.open("#{install_dir}/bin/npx", 'w') do |file|
      file.print <<-EOH
#!/bin/sh
export NODE_PATH="#{install_dir}/lib/node_modules"
exec "#{install_dir}/bin/node" "#{install_dir}/lib/node_modules/npm/bin/npx-cli.js" "$@"
      EOH
    end
  end
