...
cd /tmp; exec /opt/infisical/embedded/bin/chpst ${privilege_drop} -U ${psql_user} /usr/bin/env PGSSLCOMPRESSION=0 /opt/infisical/embedded/bin/psql -p ${psql_port} -h ${psql_host} -d ${psql_dbname} "$@"
      EOH
    end
  end
