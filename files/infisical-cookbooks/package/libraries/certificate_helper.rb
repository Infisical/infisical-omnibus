...
class CertificateHelper
  ...
  #     - ignore if pointing to /etc/infisical/trusted-certs
  ...
  def link_certificates
    ...
    unless rehash_status.zero?
      LoggingHelper.warning('Rehashing of trusted certificates present in `/etc/infisical/trusted-certs` failed. If on a FIPS-enabled machine, ensure `c_rehash` binary is available in $PATH.')
      return
    end
  ...
  def update_permissions
    ...
    FileUtils.chmod(0o644, file_list)
  end
  ...
  def raise_msg(file)
    raise "ERROR: Not a certificate: #{File.realpath(file)}. Move it from #{File.realpath('..',
                                                                                          file)} to a different location and reconfigure again."
  end
end
