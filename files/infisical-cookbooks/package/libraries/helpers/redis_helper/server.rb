...
module RedisHelper
  class Server < RedisHelper::Base
    def installed_version
      ...
      command = '/opt/infisical-core/embedded/bin/redis-server --version'
      ...
      unless version_match
        raise "Execution of the command `#{command}` generated unexpected output `#{command_output.strip}`"
      end
    ...
    def running_version
      ...
      commands = ['/opt/infisical-core/embedded/bin/redis-cli']
    ...
    def redis_cli_tls_options(args)
      ...
      unless client_cert_and_key_available?
        raise "Redis TLS client authentication requires redis['tls_cert_file'] and redis['tls_key_file'] options"
      end
