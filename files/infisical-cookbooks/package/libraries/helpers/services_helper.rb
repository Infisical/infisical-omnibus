...
module Services
  ...
  class << self
    ...
    def service_status(service)
      Infisical[:node].read(*service_attribute_path(service), 'enable')
    end
    ...
    def set_service_status(service, enabled)
      Infisical[:node].write(:default, *service_attribute_path(service), 'enable', enabled)
    end
    ...
    def service_attribute_path(service_name)
      ...
      return ['monitoring', service] if Infisical[:node]['monitoring']&.attribute?(service)
      return [service] if Infisical[:node].attribute?(service)
