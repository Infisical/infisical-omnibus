...
module SettingsDSL
  ...
  def generate_secrets(_node_name, path = SecretsHelper::SECRETS_FILE)
    # Infisical['node'][SecretsHelper::SKIP_GENERATE_SECRETS_CHEF_ATTR] is set to
    # true  if we are calling from the 'infisical-ctrl generate-secrets' command
    ...
    # false.
    return if Infisical['node'][SecretsHelper::SKIP_GENERATE_SECRETS_CHEF_ATTR] == true

    force_write_secrets = !Infisical['node'][SecretsHelper::SECRETS_FILE_CHEF_ATTR].nil?

    # guards against creating secrets on non-bootstrap node
    SecretsHelper.read_infisical_secrets(path)
    generate_default_secrets = Infisical['package']['generate_default_secrets'] != false
    ...
    if Infisical['package']['generate_secrets_json_file'] == false && !force_write_secrets
      ...
      warning_message = <<~EOS
        ...
        This results in secrets not persisting across `infisical-ctl reconfigure` runs and can cause issues with functionality.
      EOS

      LoggingHelper.warning(warning_message)
    else
      Chef::Log.info("Generating #{path} file")
      SecretsHelper.write_to_infisical_secrets(path)
    end
  end
