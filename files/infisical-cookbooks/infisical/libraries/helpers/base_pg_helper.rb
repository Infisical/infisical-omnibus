...
class BasePgHelper < BaseHelper
  ...
  attr_reader :node, :built_connection_info
  ...
  def node_attributes
    ...
    return node['infisical'][node_attribute_key] if node['infisical'].key?(node_attribute_key)
  ...
  def bootstrapped?
    ...
    # As part of https://infisical.com/infisical-org/omnibus-infisical/issues/2078 services are
    ...
    if node['infisical'].key?(node_attribute_key)
      return File.exist?(File.join(node['infisical'][node_attribute_key]['dir'], 'data',
                                   'PG_VERSION'))
    end
  ...
  def database_version
    ...
    version_file = node['infisical'].key?(node_attribute_key) ? "#{@node['infisical'][node_attribute_key]['dir']}/data/PG_VERSION" : "#{@node[node_attribute_key]['dir']}/data/PG_VERSION"
  ...
  def geo_config
    ::File.join(config_dir, 'infisical-geo.conf')
  end
