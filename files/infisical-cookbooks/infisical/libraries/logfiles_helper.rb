...
class LogfilesHelper < AccountHelper
  ...
  def service_parent(service)
    available_settings = Infisical.settings
  ...
  def logging_options(service)
    node['infisical']['logging'].to_hash.merge(service_settings(service).to_hash)
  end
  ...
  def log_group(service)
    if log_group = service_settings(service)['log_group']
      ...
      node['infisical']['logging']['log_group']
    end
  end
  ...
  def logrotate_services_list
    ...
    logrotate_services.each do |svc|
      # In `Infisical.settings`, settings aren't hyphenated, but use underscores.
      setting_name = svc.tr('-', '_')

      unless available_settings.include?(setting_name)
        raise "Service #{svc} was specified in logrotate['services'], but is not a valid service."
      end
